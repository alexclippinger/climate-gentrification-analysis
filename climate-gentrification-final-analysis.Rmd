...
tokens1 <- tokens_select(toks, pattern = stop_vec, selection = "remove")
...
```{r convert_dfm}
toks2 <- tokens_ngrams(tokens1, n=3)
dfm2 <- dfm(toks2)
...
dfm2 <- dfm(toks2)
dfm2 <- dfm_remove(dfm2, pattern = c(stop_vec))
dfm12 <- dfm_select(dfm2, pattern = c("undesirable"))
freq_words2 <- textstat_frequency(dfm2,20)
freq_words2$token <- rep("trigram", 20)
freq_words2
#tokens1 <- tokens_select(tokens1,pattern = stopwords("en"), selection = "remove")
```
...
```{r}
MIA_cg <- c("undesirable", "climate gentrification")
toks_inside <- tokens_keep(tokens1, pattern = MIA_cg, window = 20)
toks_inside <- tokens_remove(toks_inside, pattern = MIA_cg) # remove the keywords
toks_outside <- tokens_remove(tokens1, pattern = MIA_cg, window = 20)
```
